...
private:
    ...
    // --- 定数 ---
    static inline const float kPlayerSpeed = 2.0f;                  // プレイヤーの基本移動速度
    static inline const float kPlayerSpeedMax = kPlayerSpeed * 2.f; // 移動速度の最大値
    static inline const float kGravity = 0.98f;                     // 重力加速度
    static inline const float kLimitFallSpeed = kGravity * 10.f;    // 落下速度制限
    static inline const float kJumpAcceleration = 10.00f;           // ジャンプの加速度
    static inline const float kWidth = 30;                          // 当たり判定幅
    static inline const float kHeight = 30;                         // 当たり判定高さ
    static inline const float kBlank = 1.f;                         // 当たり判定余白
